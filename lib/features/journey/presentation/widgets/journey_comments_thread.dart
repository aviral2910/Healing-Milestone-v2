...
class JourneyCommentsThread extends ConsumerStatefulWidget {
  final JourneyMilestoneModel milestone;

  const JourneyCommentsThread({Key? key, required this.milestone}) : super(key: key);

  @override
  ConsumerState<JourneyCommentsThread> createState() => _JourneyCommentsThreadState();
}
...
class _JourneyCommentsThreadState extends ConsumerState<JourneyCommentsThread> {
  ...
  Future<void> _submitComment() async {
    ...
    await ref.read(journeyRepositoryProvider).addMilestoneComment(widget.milestone.id, text);
  ...
  Widget build(BuildContext context) {
    final commentsAsync = ref.watch(
      paginatedJourneyCommentsProvider(widget.milestone.id),
    );
                ...
                return NotificationListener<ScrollNotification>(
                  onNotification: (ScrollNotification scrollInfo) {
                    if (scrollInfo.metrics.pixels >= scrollInfo.metrics.maxScrollExtent - 50) {
                      ref.read(paginatedJourneyCommentsProvider(widget.milestone.id).notifier).fetchNextPage();
                    }
                    return false;
                  },
                          ...
                          TextButton(
                            onPressed: () async {
                              ...
                              await ref
                                  ...
                                  .deleteMilestoneComment(storyId, comment.commentId);
      ...
      builder: (context, scrollController) {
        return Column(
          children: [
            ...
            Expanded(child: JourneyCommentsThread(milestone: story)),
          ],
        );
      },
    ),
