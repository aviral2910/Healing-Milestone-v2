...
class LogMilestoneOverlay extends StatefulWidget {
  ...
  static Future<void> show(BuildContext context, {String? journeyId, JourneyMilestoneModel? milestone}) {
    return showGeneralDialog(
      ...
      pageBuilder: (context, animation, secondaryAnimation) {
        return LogMilestoneOverlay(initialJourneyId: journeyId, initialMilestone: milestone);
      },
...
class _LogMilestoneOverlayState extends State<LogMilestoneOverlay> {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: isSelected ? color.withValues(alpha: 0.15) : theme.colorScheme.surface.withValues(alpha: 0.5),
          borderRadius: BorderRadius.circular(24),
          border: Border.all(
            color: isSelected ? color : theme.dividerColor.withValues(alpha: 0.3),
  ...
  Widget build(BuildContext context) {
    ...
    Color primaryGlow = _selectedEmotion != null ? _getEmotionColor(_selectedEmotion!) : theme.colorScheme.primary;

    // Check if it's a floating check-in (no journey ID from constructor and no journey ID from existing milestone)
    final bool isFloating = widget.initialJourneyId == null && (widget.initialMilestone?.journeyId == null);
                          ...
                          Row(
                            children: [
                              for (final val in MilestoneVisibility.values)
                                Expanded(
                                  child: Padding(
                                    padding: EdgeInsets.only(right: val != MilestoneVisibility.values.last ? 12.0 : 0),
                                    child: Builder(
                                      builder: (context) {
                                        final isSelected = _selectedVisibility == val;
                                        return GestureDetector(
                                          onTap: () => setState(() => _selectedVisibility = val),
                                          child: AnimatedContainer(
                                            ...
                                            child: Column(
                                              children: [
                                                Icon(
                                                  val == MilestoneVisibility.private ? Icons.lock_outline_rounded : 
                                                  val == MilestoneVisibility.anonymous ? Icons.masks_outlined :
                                                ...
                                                Text(
                                                  val.name.substring(0, 1).toUpperCase() + val.name.substring(1),
                                ...
                                child: Padding(
                                  ...
                                  child: Row(
                                    ...
                                    children: [
                                      ...
                                      Text('Add Voice Note', style: TextStyle(color: theme.colorScheme.primary, fontWeight: FontWeight.w500)),
                                    ],
                                  ),
                                ),
                              ),
                            ...
                            builder: (context, ref, child) {
                              return FilledButton(
                                      ...
                                      try {
                                        ...
                                        if (isEditing) {
                                          await repo.updateMilestone(
                                            milestoneId: widget.initialMilestone!.id,
                                            emotionStatus: _selectedEmotion!,
                                            content: _contentController.text.trim().isEmpty ? null : _contentController.text.trim(),
                                            visibility: isFloating ? _selectedVisibility : null,
                                          );
                                        } else {
                                          await repo.createMilestone(
                                            journeyId: widget.initialJourneyId,
                                            emotionStatus: _selectedEmotion!,
                                            content: _contentController.text.trim().isEmpty ? null : _contentController.text.trim(),
                                            visibility: isFloating ? _selectedVisibility : MilestoneVisibility.public,
                                          );
                                        }
                                        
                                        // Invalidate providers
                                        final targetJourneyId = widget.initialJourneyId ?? widget.initialMilestone?.journeyId;
                                        if (targetJourneyId != null) {
                                          ref.invalidate(journeyMilestonesProvider(targetJourneyId));
                                        } else {
                                ...
                                child: _isSubmitting
                                  ...
                                  : Text(
                                      isEditing ? 'Save Changes' : (widget.initialJourneyId != null ? 'Log Step' : 'Post Check-In'),
